...
def obtener_precios(conn, fecha, tickers):
    ...
    for ticker in tickers:
        ...
        max_intentos = 60  # Máximo número de días hacia atrás que buscaremos

        while intentos < max_intentos:
            query = f"SELECT valor FROM PRECIOS WHERE ticker = '{ticker}' AND fecha = '{fecha_actual.strftime('%Y-%m-%d')}' LIMIT 1;"
        ...
        if intentos >= max_intentos:
            precios[ticker] = np.nan  # No se encontró un precio en el rango de fechas
