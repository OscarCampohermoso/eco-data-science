...
def obtener_datos_trimestrales(mm, aa, empresa_busqueda='CENCOSUD S.A.'):
    ...
    # Pausa para prevenir bloqueos por hacer peticiones muy seguidas
    time.sleep(5)
    ...
    for fila in soup.find_all('tr'):
        ...
        if len(columnas) == 6:
            ...
            if empresa_busqueda.lower() == razon_social.lower():
                trimestre_financiero = f'{aa}-{mm}-31' if mm in [
                    '03', '12'] else f'{aa}-{mm}-30'
                datos.append({
...
def guardar_datos(df, nombre_db='datos_eeff.db', nombre_tabla='FECHAS_EEFF'):
    ...
    columnas_necesarias = ['Trimestre_Estado_Financiero',
                           'Fecha_Primer_envio', 'Fecha_ultimo_envio']
